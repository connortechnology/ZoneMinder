...
bool EventStream::sendFrame(Microseconds delta_us) {
  ...
  if ( type == STREAM_MPEG ) {
    ...
    if (send_raw) {
      ...
      // When the caller decoded straight to the send size, prepareImage returns
      // the image untouched and it is already the encoder's target. Otherwise
      // the jpeg encoder below does the scaling, as it has here all along.
      int l_width  = pre_scaled_by ? send_image->Width()
                                   : floor(send_image->Width()  * scale / ZM_SCALE_BASE);
      int l_height = pre_scaled_by ? send_image->Height()
                                   : floor(send_image->Height() * scale / ZM_SCALE_BASE);
