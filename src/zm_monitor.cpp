...
std::string Capturing_Strings[] = {"Unknown", "None", "On demand", "Always"};

std::string Analysing_Strings[] = {"Unknown", "None", "Always"};

std::string Recording_Strings[] = {"Unknown", "None", "On motion", "Always"};

std::string Decoding_Strings[] = {
    "Unknown", "None", "On demand", "Keyframes", "Keyframes + Ondemand",
    "Always"};

std::string RTSP2Web_Strings[] = {"HLS", "MSE", "WebRTC"};

std::string Go2RTC_Strings[] = {"WebRTC", "MSE", "RTSP", "HLS", "MJPEG"};
...
//label_format
  ...
  shared_analysis_timestamps(nullptr),
  shared_images(nullptr),
  shared_analysis_images(nullptr),
...
void Monitor::Load(MYSQL_ROW dbrow, bool load_zones = true, Purpose p = QUERY) {
  ...
  capture_delay = (dbrow[col] && atof(dbrow[col]) > 0.0)
                      ? std::chrono::duration_cast<Microseconds>(
                            FPSeconds(1 / atof(dbrow[col])))
                      : Microseconds(0);
  col++;
  alarm_capture_delay = (dbrow[col] && atof(dbrow[col]) > 0.0)
                            ? std::chrono::duration_cast<Microseconds>(
                                  FPSeconds(1 / atof(dbrow[col])))
                            : Microseconds(0);
  col++;
  ...
  /* "Protocol, Method, Options, User, Pass, Host, Port, Path, SecondPath,
   * Width, Height, Colours, Palette, Orientation+0, Deinterlacing, " */
  ...
  col++;
  width = (orientation == ROTATE_90 || orientation == ROTATE_270)
              ? camera_height
              : camera_width;
  height = (orientation == ROTATE_90 || orientation == ROTATE_270)
               ? camera_width
               : camera_height;
  ...
  if (section_length < min_section_length) {
    ...
    Warning("Section length %" PRId64 " < Min Section Length %" PRId64
            ". This is invalid.",
            static_cast<int64>(Seconds(section_length).count()),
            static_cast<int64>(Seconds(min_section_length).count()));
  }
  event_close_mode =
      static_cast<Monitor::EventCloseMode>(dbrow[col] ? atoi(dbrow[col]) : 0);
  col++;
  ...
  col++;
  signal_check_colour =
      strtol(dbrow[col][0] == '#' ? dbrow[col] + 1 : dbrow[col], 0, 16);
  col++;

  colour_val =
      rgb_convert(signal_check_colour,
                  ZM_SUBPIX_ORDER_BGR); /* HTML colour code is actually BGR in
                                           memory, we want RGB */
  ...
  grayscale_val =
      signal_check_colour & 0xff; /* Clear all bytes but lowest byte */
  ...
  col++;
  std::string mqtt_subscriptions_string =
  ...
  // Instead initial failure to capture will cause a loss of signal change which
  ...
  if (purpose != QUERY) {
    ...
    if (config.record_diag_images) {
      if (config.record_diag_images_fifo) {
        diag_path_ref = stringtf("%s/diagpipe-r-%d.jpg",
                                 staticConfig.PATH_SOCKS.c_str(), id);
        diag_path_delta = stringtf("%s/diagpipe-d-%d.jpg",
                                   staticConfig.PATH_SOCKS.c_str(), id);
  ...
  switch (type) {
    case LOCAL: {
      ...
      camera = zm::make_unique<LocalCamera>(
          this, device, channel, format, v4l_multi_buffer,
          v4l_captures_per_frame, method, camera_width, camera_height, colours,
          palette, brightness, contrast, hue, colour, purpose == CAPTURE,
          record_audio, extras);
#else
    ...
    case REMOTE: {
      if (protocol == "http") {
        camera = zm::make_unique<RemoteCameraHttp>(
            this, method, host, port, path, user, pass, camera_width,
    ...
    case FILE: {
      camera = zm::make_unique<FileCamera>(
          this, path.c_str(), camera_width, camera_height, colours, brightness,
          contrast, hue, colour, purpose == CAPTURE, record_audio);
      break;
    }
    case FFMPEG: {
    ...
    case VNC: {
      ...
      camera = zm::make_unique<VncCamera>(
          this, host.c_str(), port.c_str(), user.c_str(), pass.c_str(), width,
          height, colours, brightness, contrast, hue, colour,
          purpose == CAPTURE, record_audio);
#else   // HAVE_LIBVNC
      Fatal(
          "You must have libvnc installed to use VNC cameras for monitor id %d",
          id);
#endif  // HAVE_LIBVNC
      break;
    }
    default: {
      Fatal("Tried to load unsupported camera type %d for monitor %u",
            int(type), id);
      break;
    }
  }
...
bool Monitor::connect() {
  ...
  mem_size = sizeof(SharedData)
             ...
             + (image_buffer_count*image_size) // analysis_images
             + (image_buffer_count*sizeof(AVPixelFormat)) // per-slot capture pix fmt
             + (image_buffer_count*sizeof(AVPixelFormat)) // per-slot analysis pix fmt
  ...
  Debug(1,
        ...
        "total=%jd",
        sizeof(SharedData), sizeof(TriggerData), zone_count, sizeof(int),
        sizeof(VideoStoreData), (image_buffer_count * sizeof(struct timeval)),
        image_buffer_count, image_size,
        static_cast<size_t>((image_buffer_count * image_size)),
        image_buffer_count, image_size,
        static_cast<size_t>((image_buffer_count * image_size)),
        image_buffer_count, sizeof(AVPixelFormat),
        static_cast<size_t>(image_buffer_count * sizeof(AVPixelFormat)),
  ...
  if (map_fd < 0) {
    Info("Can't open memory map file %s: %s", mem_file.c_str(),
         strerror(errno));
    return false;
  } else {
  ...
  if (fstat(map_fd, &map_stat) < 0) {
    Error(
        "Can't stat memory map file %s: %s, is the zmc process for this "
        "monitor running?",
        mem_file.c_str(), strerror(errno));
    close(map_fd);
    map_fd = -1;
    return false;
  }
  ...
  if (map_stat.st_size != mem_size) {
    if (purpose == CAPTURE) {
      ...
      if (ftruncate(map_fd, mem_size) < 0) {
        Error("Can't extend memory map file %s to %jd bytes: %s",
              mem_file.c_str(), static_cast<intmax_t>(mem_size),
              strerror(errno));
        close(map_fd);
        map_fd = -1;
        return false;
      ...
      Error(
          "Got empty memory map file size %jd, is the zmc process for this "
          "monitor running?",
          static_cast<intmax_t>(map_stat.st_size));
      close(map_fd);
      map_fd = -1;
      return false;
    } else {
      Error("Got unexpected memory map file size %jd, expected %jd",
            static_cast<intmax_t>(map_stat.st_size),
            static_cast<intmax_t>(mem_size));
      close(map_fd);
      map_fd = -1;
...
#ifdef MAP_LOCKED
  mem_ptr = (unsigned char *)mmap(nullptr, mem_size, PROT_READ | PROT_WRITE,
                                  MAP_SHARED | MAP_LOCKED, map_fd, 0);
  if (mem_ptr == MAP_FAILED) {
    if (errno == EAGAIN) {
      Debug(
          1,
          "Unable to map file %s (%jd bytes) to locked memory, trying unlocked",
          mem_file.c_str(), static_cast<intmax_t>(mem_size));
#endif
      mem_ptr = (unsigned char *)mmap(nullptr, mem_size, PROT_READ | PROT_WRITE,
                                      MAP_SHARED, map_fd, 0);
      Debug(1, "Mapped file %s (%jd bytes) to unlocked memory",
            mem_file.c_str(), static_cast<intmax_t>(mem_size));
#ifdef MAP_LOCKED
    } else {
      Error("Unable to map file %s (%jd bytes) to locked memory (%s)",
            mem_file.c_str(), static_cast<intmax_t>(mem_size), strerror(errno));
    }
  }
#endif
  if ((mem_ptr == MAP_FAILED) or (mem_ptr == nullptr)) {
    Error("Can't map file %s (%jd bytes) to memory: %s(%d)", mem_file.c_str(),
          static_cast<intmax_t>(mem_size), strerror(errno), errno);
    close(map_fd);
    map_fd = -1;
    mem_ptr = nullptr;
    return false;
...
#else   // ZM_MEM_MAPPED
  shm_id =
      shmget((config.shm_key & 0xffff0000) | id, mem_size, IPC_CREAT | 0700);
  if (shm_id < 0) {
    Fatal("Can't shmget, probably not enough shared memory space free: %s",
          strerror(errno));
  }
  ...
  if (((unsigned long)shared_images % 64) != 0) {
    ...
    unsigned char *aligned_shared_images =
        (unsigned char *)((unsigned long)shared_images +
                          (64 - ((unsigned long)shared_images % 64)));
    Debug(3,
          "Aligning shared memory images to the next 64 byte boundary %p to %p "
          "moved %lu bytes",
  ...
  // analysis_image_pixelformats[image_buffer_count] array.
  //
  ...
  if (reinterpret_cast<unsigned char *>(analysis_image_pixelformats + image_buffer_count) > mem_ptr + mem_size) {
    Warning("We will exceed memsize by %td bytes!",
            reinterpret_cast<unsigned char *>(analysis_image_pixelformats + image_buffer_count) - (mem_ptr + mem_size));
  }
  ...
  if (purpose == CAPTURE) {
    ...
    // memset zeroed image_pixelformats/analysis_image_pixelformats above;
    // 0 is AV_PIX_FMT_YUV420P, not the AV_PIX_FMT_NONE sentinel readers
    // expect for "format not yet published". Initialise explicitly.
    for (int32_t i = 0; i < image_buffer_count; i++) {
    ...
    snprintf(video_store_data->event_file, sizeof(video_store_data->event_file),
             "nothing");
    ...
    if (mqtt_enabled) {
      ...
      for (const std::string &subscription : mqtt_subscriptions) {
        if (!subscription.empty()) mqtt->add_subscription(subscription);
      }
    }
#endif
...
bool Monitor::disconnect() {
  ...
  for (int32_t i = 0; i < image_buffer_count; i++) {
    // We delete the image because it is an object pointing to space that won't
    // be free'd.
...
Image *Monitor::GetAlarmImage() {
  // The annotated analysis frames live in the analysis ring; serve the most
  // recent one. Route the read through ReadAnalysisShmFrame so the per-slot
  // AVPixelFormat published by the analysis process is adopted before the
  // bytes are interpreted — otherwise a reader process (zms) would use
  // whatever placeholder format the buffer was constructed with, producing
  // garbled output whenever the writer used RGB24/RGBA/etc.
  int32_t index = shared_data->last_analysis_index;
  if ((index < 0) || (index >= image_buffer_count)) {
    // image_buffer_count is the "not yet published" sentinel; anything else
    // out of range is a corrupt/torn SHM value. Either way there is no
    // analysis frame to serve.
    return nullptr;
  }
  return ReadAnalysisShmFrame(index);
}
...
int Monitor::GetImage(int32_t index, int scale) {
  ...
  // Check the sentinel BEFORE the bounds check: when last_decoder_index is
  ...
  // If we are going to be modifying the snapshot before writing, then we need
  // to copy it
...
std::shared_ptr<ZMPacket> Monitor::getSnapshot(int index) {
  ...
  // written yet" (last_decoder_index is still the sentinel). Without this
...
int Monitor::GetLastReadIndex() const {
  return (shared_data->last_read_index != image_buffer_count
...
double Monitor::GetFPS() const { return get_capture_fps(); }

/* I think this returns the # of micro seconds that we should sleep in order to
 * maintain the desired analysis rate */
useconds_t Monitor::GetAnalysisRate() {
  ...
  if (!analysis_fps_limit) {
    ...
    if (last_fps_time != last_analysis_fps_time) {
      ...
      Warning("Analysis fps (%.2f) is greater than capturing fps (%.2f)",
              analysis_fps_limit, capture_fps);
    }
    return 0;
  } else if (capture_fps) {
...
void Monitor::UpdateAdaptiveSkip() {
  if (config.opt_adaptive_skip) {
    ...
    if (adaptive_skip && analysis_fps && (analysis_fps < capturing_fps)) {
      Info(
          "Analysis fps (%.2f) is lower than capturing fps (%.2f), disabling "
          "adaptive skip feature",
          analysis_fps, capturing_fps);
      adaptive_skip = false;
    } else if (!adaptive_skip &&
               (!analysis_fps || (analysis_fps >= capturing_fps))) {
...
void Monitor::DumpZoneImage(const char *zone_string) {
  ...
  if (zone_string) {
    if (!Zone::ParseZoneString(zone_string, exclude_id, extra_colour,
                               extra_zone)) {
  ...
  if (((!staticConfig.SERVER_ID) || (staticConfig.SERVER_ID == server_id)) &&
      mem_ptr) {
  ...
  for (const Zone &zone : zones) {
    if (exclude_id && (!extra_colour || !extra_zone.GetVertices().empty()) &&
        zone.Id() == exclude_id) {
      continue;
    }

    Rgb colour;
    if (exclude_id && extra_zone.GetVertices().empty() &&
        zone.Id() == exclude_id) {
      colour = extra_colour;
    } else {
      if (zone.IsActive()) {
...
bool Monitor::CheckSignal(const Image *image) {
  if (signal_check_points <= 0) return true;
  ...
  Debug(1, "SignalCheck: %d points, colour_val(%d)", signal_check_points,
        colour_val);
  return false;
...
void Monitor::CheckAction() {
  if (shared_data->action) {
    // Can there be more than 1 bit set in the action?  Shouldn't these be
    // elseifs?
...
void Monitor::UpdateFPS() {
  ...
  // If we are too fast, we get div by zero. This seems to happen in the case of
  ...
  if (elapsed > Seconds(1)) {
    ...
    if ( fps_report_interval and
      ...
      if (mqtt)
        mqtt->send(
            stringtf("Capturing at %.2lf fps, capturing bandwidth %ubytes/sec "
    ...
    if (db_elapsed > kStatusUpdateInterval) {
      std::string sql = stringtf(
          ...
          "(%u, 'Connected',%.2lf, %u, %.2lf, NOW()) ON DUPLICATE KEY "
          "UPDATE Status='Connected', CaptureFPS = %.2lf, CaptureBandwidth=%u, "
          "AnalysisFPS = %.2lf, UpdatedOn=NOW()",
...
// This idea is that we should be analysing as close to the capture frame as
// possible. This method should process as much as possible before returning
//
  ...
  { // scope for event lock
    ...
    if (trigger_data->trigger_state != TriggerState::TRIGGER_OFF) {
      ...
      if (signal_change) {
        ...
        if (!signal) {
          if (event) {
            ...
            Info("%s: %03d - Closing event %" PRIu64 ", signal loss",
      ...
      if (signal) {
        ...
        // immediately by linked monitors
        ...
        if (score) {
          if ((state == IDLE) || (state == PREALARM)) {
            ...
            Debug(1,
                  "%s: %03d - Alarmed frame while in alert state. Consecutive "
                  "alarmed frames left to return to alarm state: %03d",
                  name.c_str(), analysis_image_count,
                  alert_to_alarm_frame_count);
            if (alert_to_alarm_frame_count == 0) {
              Info("%s: %03d - ExtAlm - Gone back into alarm state",
                   name.c_str(), analysis_image_count);
              shared_data->state = state = ALARM;
            }
          } else {
          ...
          if ((motion_score >= 0) or (shared_data->analysing != ANALYSING_ALWAYS)) {
            ...
            if (state == ALARM) {
              ...
              if ((analysis_image_count - last_alarm_count) >
                  post_event_count) {
                shared_data->state = state = IDLE;
            ...
            Debug(1,
                  "State %d %s because "
                  "analysis_image_count(%d)-last_alarm_count(%d) = %d > "
                  "post_event_count(%d) and timestamp.tv_sec(%" PRIi64
                  ") - recording.tv_src(%" PRIi64
                  ") >= min_section_length(%" PRIi64 ")",
        ...
        if (event) {
          ...
          if (event->Duration() >= min_section_length) {
            ...
            if ((event->Duration() >= section_length) and
                (event->Frames() < Seconds(min_section_length).count())) {
            ...
            if (shared_data->recording == RECORDING_ALWAYS) {
              if (event_close_mode == CLOSE_ALARM) {
                ...
                if (state == ALARM) {
                  ...
                  if (event->AlarmFrames() < alarm_frame_count) {
                    ...
                    Debug(1,
                          "Not Closing event %" PRIu64
                          ". Event alarm frames %d < alarm_frame_count %d",
                          event->Id(), event->AlarmFrames(), alarm_frame_count);
                  }
                } else if (state == IDLE) {
                  if (event->AlarmFrames() > alarm_frame_count and
                      (analysis_image_count - last_alarm_count >
                       post_event_count)) {
                    Info("%s: %03d - Closing event %" PRIu64 ", alarm end",
                         name.c_str(), analysis_image_count, event->Id());
                    closeEvent();
                  } else if (event->Duration() > section_length) {
                  ...
                  Debug(1,
                        "Not closing event because state==%s and event "
                        "AlarmFrames %d >= %d",
                        State_Strings[state].c_str(), event->AlarmFrames(),
                        alarm_frame_count);
                }
              ...
              if (event->Duration() > section_length or (IDLE==state and (analysis_image_count - last_alarm_count > post_event_count))) {
                ...
                Debug(1,
                      "Not Closing event %" PRIu64
                      ", continuous end, alarm begins. Event alarm frames %d < "
                      "alarm_frame_count %d",
                      event->Id(), event->AlarmFrames(), alarm_frame_count);
              }
            } else {
        ...
        if (!event) {
          if ((shared_data->recording == RECORDING_ALWAYS) or
      ...
      Debug(3, "trigger == off");
      if (event) {
        Info("%s: %03d - Closing event %" PRIu64 ", trigger off", name.c_str(),
             analysis_image_count, event->Id());
        closeEvent();
      }
      shared_data->state = state = IDLE;
      EndAlarmActions();
...
void Monitor::ReloadLinkedMonitors() {
  Debug(1, "Reloading linked monitors for monitor %s, '%s'", name.c_str(),
        linked_monitors_string.c_str());
  delete linked_monitors;
  linked_monitors = nullptr;
...
std::vector<std::shared_ptr<Monitor>> Monitor::LoadMonitors(
    const std::string &where, Purpose purpose) {
...
std::vector<std::shared_ptr<Monitor>> Monitor::LoadRemoteMonitors(
    const char *protocol, const char *host, const char *port, const char *path,
    Purpose purpose) {
...
int Monitor::Capture() {
  ...
  if (captureResult < 0) {
    ...
    // Not sure what to do here.  We will close monitor and kill analysis_thread
    // but what about rtsp server?
    Rgb signalcolor;
    ...
    shared_data->signal =
        true;  // Assume if getting packets that we are getting something
    ...
    if (packet->codec_type == AVMEDIA_TYPE_VIDEO) {
      packet->packet->stream_index =
          video_stream_id;  // Convert to packetQueue's index
      if (video_fifo) {
        if (packet->keyframe) {
          ...
          video_fifo->write(
              ...
              stream->codecpar->extradata_size, packet->pts);
        }
      ...
      if (record_audio and (packetqueue.packet_count(video_stream_id) or event)) {
        ...
        packet->packet->stream_index =
            audio_stream_id;  // Convert to packetQueue's index
      ...
      Debug(1, "Unknown codec type %d %s", packet->codec_type,
            av_get_media_type_string(packet->codec_type));
      return 1;
    }  // end if audio
  ...
  // Icon: I'm not sure these should be here. They have nothing to do with
  // capturing
...
bool Monitor::setupConvertContext(const AVFrame *input_frame, const Image *image) {
  ...
  convert_context = sws_getContext(
      ...
      image->Height(), imagePixFormat, SWS_BICUBIC, nullptr, nullptr, nullptr);
...
Image *Monitor::ReadAnalysisShmFrame(unsigned int index) {
  // Same contract as ReadShmFrame, but for the analysis ring: adopt the
  // per-slot format the analysis process recorded into
  // analysis_image_pixelformats[] before interpreting the slot's bytes.
  // The same untrusted-SHM checks apply.
  AVPixelFormat fmt = analysis_image_pixelformats[index];
  if (fmt == AV_PIX_FMT_NONE || analysis_image_buffer[index]->PixFormat() == fmt) {
    return analysis_image_buffer[index];
  }
  unsigned int probe_colours, probe_subpix;
  if (!zm_colours_from_pixformat(fmt, probe_colours, probe_subpix)) {
    Warning("ReadAnalysisShmFrame: ignoring unsupported pixelformat %d in slot %u; keeping current %s",
            fmt, index, zm_get_pix_fmt_name(analysis_image_buffer[index]->PixFormat()));
    return analysis_image_buffer[index];
  }
  int required = av_image_get_buffer_size(fmt, width, height, 32);
  if (required < 0 || static_cast<size_t>(required) > shm_slot_size) {
    Warning("ReadAnalysisShmFrame: format %s requires %d bytes but slot %u capacity is %zu; "
            "keeping current %s to avoid overrun",
            zm_get_pix_fmt_name(fmt), required, index, shm_slot_size,
            zm_get_pix_fmt_name(analysis_image_buffer[index]->PixFormat()));
    return analysis_image_buffer[index];
  }
  analysis_image_buffer[index]->AVPixFormat(fmt);
  return analysis_image_buffer[index];
}
...
std::string Monitor::Substitute(const std::string &format, SystemTimePoint ts_time) const {
  ...
  strftime(label_time_text, sizeof(label_time_text), format.c_str(),
      ...
      switch (*(s_ptr + 1)) {
        ...
        case 'f':
          ...
          Centiseconds centi_sec = std::chrono::duration_cast<Centiseconds>(
              ts_time.time_since_epoch() -
              std::chrono::duration_cast<Seconds>(ts_time.time_since_epoch()));
          rc = snprintf(d_ptr, max_len, "%02lld",
                        static_cast<long long int>(centi_sec.count()));
          found_macro = true;
          break;
      }
...
Event * Monitor::openEvent(
  ...
  if (!starting_packet) {
    Warning("Unable to get starting packet");
    // Every other path out of here hands start_it to the Event, which frees it
    // in ~Event. Leaking it leaves a registered iterator pinned to the front of
    // the queue, which stops clearPackets() removing anything for the life of
    // the process.
  ...
  strncpy(shared_data->alarm_cause, cause.c_str(),
          sizeof(shared_data->alarm_cause) - 1);
...
unsigned int Monitor::Colours() const {
  return camera ? camera->Colours() : colours;
}
unsigned int Monitor::SubpixelOrder() const {
  return camera ? camera->SubpixelOrder() : 0;
}
...
int Monitor::PrimeCapture() {
  ...
  Debug(2,
        "Video stream id is %d, audio is %d, minimum_packets to keep in buffer "
        "%d",
  ...
  if (rtsp_server) {
    if (video_stream_id >= 0) {
      ...
      snprintf(shared_data->video_fifo_path,
               sizeof(shared_data->video_fifo_path) - 1, "%s/video_fifo_%u.%s",
               staticConfig.PATH_SOCKS.c_str(), id,
    ...
    if (record_audio and (audio_stream_id >= 0)) {
      ...
      if (audioStream && CODEC(audioStream)) {
        snprintf(shared_data->audio_fifo_path,
                 sizeof(shared_data->audio_fifo_path) - 1,
                 "%s/audio_fifo_%u.%s", staticConfig.PATH_SOCKS.c_str(), id,
...
// Close can be called multiple times, for example before object delete, and
...
// Wait for camera to get an image, and then assign it as the base reference
// image. So this should be done as the first task in the analysis thread
// startup. This function is deprecated.
