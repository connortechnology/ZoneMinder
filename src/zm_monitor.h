...
class Monitor : public std::enable_shared_from_this<Monitor> {
 ...
 public:
  ...
  typedef enum { ANALYSING_NONE = 1, ANALYSING_ALWAYS } AnalysingOption;
  ...
  typedef enum { UNKNOWN = 0, IDLE, PREALARM, ALARM, ALERT } State;
 ...
 public:
  ...
  typedef struct {
    ...
    uint8_t signal;             /* +92   */
    uint8_t format;             /* +93   */
    uint8_t audio_level;        /* +94   0-100, written by the capture thread */
    uint8_t audio_alarm;        /* +95   audio_level crossed AudioThreshold */
    ...
    /*
     ** This keeps 32bit time_t and 64bit time_t identical and compatible as
     *long as time is before 2038.
     ** Shared memory layout should be identical for both 32bit and 64bit and is
     *multiples of 16.
     ** Because startup_time is 64bit it may be aligned to a 64bit boundary.  So
    ...
    union {                /* +112   */
      time_t startup_time; /* When the zmc process started.  zmwatch uses this
                              ...
                              without getting any images */
      uint64_t extrapad1;
    };
    union {                  /* +120   */
      time_t heartbeat_time; /* Constantly updated by zmc.  Used to determine if
                                the process is alive or hung or dead */
      uint64_t extrapad2;
    };
    union { /* +128   */
  ...
  // TODO: Technically we can't exclude this struct when people don't have
  // avformat as the Memory.pm module doesn't know about avformat
  // sizeOf(VideoStoreData) expected to be 4104 bytes on 32bit and 64bit
  typedef struct {
    uint32_t size;
    uint64_t current_event;
    char event_file[4096];
    timeval recording;  // used as both bool and a pointer to the timestamp when
 ...
 public:
  class MonitorLink {
   ...
   public:
    ...
    inline uint32_t lastFrameScore() { return shared_data->last_frame_score; }
  ...
  std::string
      event_prefix;  // The prefix applied to event names as they are created
  ...
  int32_t
      image_buffer_count;  // Size of circular image buffer, kept in /dev/shm
  int32_t
      max_image_buffer_count;  // Max # of video packets to keep in packet queue
  int warmup_count;     // How many images to process before looking for events
  int pre_event_count;  // How many images to hold and prepend to an alarm event
  int post_event_count;      // How many unalarmed images must occur before the
                             // alarm state is reset
  int stream_replay_buffer;  // How many frames to store to support DVR
                             // functions, IGNORED from this object, passed
                             // directly into zms now
  Seconds section_length;    // How long events should last in continuous modes
  bool section_length_warn;  // Whether to log a warning when a motion event
                             // exceeds desired section_length
  Seconds min_section_length;        // Minimum event length when using
                                     // event_close_mode == ALARM
  bool startstop_on_section_length;  // Whether to start/stop events on time %
                                     // section_length
  ...
  Microseconds alarm_capture_delay;  // How long we wait between capture frames
                                     // when in alarm state
  int alarm_frame_count;  // How many alarm frames are required before an event
                          // is triggered
  int alert_to_alarm_frame_count;  // How many alarm frames (consecutive score
                                   // frames) are required to return alarm from
                                   // alert
  // value for now is the same number configured in alarm_frame_count, maybe
  // getting his own parameter some day
  int fps_report_interval;  // How many images should be captured/processed
                            // between reporting the current FPS
  int ref_blend_perc;  // Percentage of new image going into reference image.
  int alarm_ref_blend_perc;  // Percentage of new image going into reference
                             // image during alarm.
  bool track_motion;  // Whether this monitor tries to track detected motion
  int signal_check_points;  // Number of points in the image to check for signal
  Rgb signal_check_colour;  // The colour that the camera will emit when no
  ...
  int importance;     // Importance of this monitor, affects Connection logging
                      // errors.
 ...
 public:
  ...
  inline bool Active() const { return shared_data->analysing; }
  int64_t getLastViewed() {
    if (shared_data && shared_data->valid) return shared_data->last_viewed_time;
    return 0;
  }
  void setLastViewed() { setLastViewed(std::chrono::system_clock::now()); }
  void setLastViewed(SystemTimePoint new_time) {
    if (shared_data && shared_data->valid)
      shared_data->last_viewed_time = static_cast<int64>(
          std::chrono::duration_cast<Seconds>(new_time.time_since_epoch())
              .count());
  }
  bool hasViewers() {
    if (shared_data && shared_data->valid) {
      SystemTimePoint now = std::chrono::system_clock::now();
      int64 intNow = static_cast<int64>(
          std::chrono::duration_cast<Seconds>(now.time_since_epoch()).count());
      Debug(3, "Last viewed %" PRId64 " seconds ago",
            intNow - shared_data->last_viewed_time);
      return (((!shared_data->last_viewed_time) or
  ...
  inline double Latitude() const {
    return shared_data ? shared_data->latitude : latitude;
  }
  inline double Longitude() const {
    return shared_data ? shared_data->longitude : longitude;
  }
  ...
  */
  ...
  uint64_t GetVideoWriterEventId() const {
    return video_store_data->current_event;
  }
  void SetVideoWriterEventId(uint64_t p_event_id) {
    video_store_data->current_event = p_event_id;
  }

  SystemTimePoint GetVideoWriterStartTime() const {
    return SystemTimePoint(
        zm::chrono::duration_cast<Microseconds>(video_store_data->recording));
  }
  void SetVideoWriterStartTime(SystemTimePoint t) {
    video_store_data->recording =
        zm::chrono::duration_cast<timeval>(t.time_since_epoch());
  }
  ...
  double GetFPS() const;
  void UpdateFPS();
  void ForceAlarmOn(int force_score, const char *force_case,
  ...
  struct transfer
{
  ...
  static std::vector<std::shared_ptr<Monitor>> LoadMonitors(
      const std::string &sql,
      Purpose purpose);  // Returns # of Monitors loaded, 0 on failure.
#if ZM_HAS_V4L2
  static std::vector<std::shared_ptr<Monitor>> LoadLocalMonitors(
      const char *device, Purpose purpose);
#endif  // ZM_HAS_V4L2
  static std::vector<std::shared_ptr<Monitor>> LoadRemoteMonitors(
      const char *protocol, const char *host, const char *port,
      const char *path, Purpose purpose);
  static std::vector<std::shared_ptr<Monitor>> LoadFileMonitors(
      const char *file, Purpose purpose);
  static std::vector<std::shared_ptr<Monitor>> LoadFfmpegMonitors(
      const char *file, Purpose purpose);
  static std::shared_ptr<Monitor> Load(unsigned int id, bool load_zones,
                                       Purpose purpose);
  void Load(MYSQL_ROW dbrow, bool load_zones, Purpose purpose);
  // void writeStreamImage( Image *image, struct timeval *timestamp, int scale,
  // int mag, int x, int y ); void StreamImages( int scale=100, int maxfps=10,
  // time_t ttl=0, int msq_id=0 ); void StreamImagesRaw( int scale=100, int
  // maxfps=10, time_t ttl=0 ); void StreamImagesZip( int scale=100, int
  // maxfps=10, time_t ttl=0 ); void StreamMpeg( const char *format, int
  // scale=100, int maxfps=10, int bitrate=100000 );
